        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <form role="form" id="frmModify" method="post" name="frmBoard" >
					...
					<div class="form-group">
						<label>운동 부위</label>
							<input class="form-control" name="workout_target" value='<c:out value="${workout.workout_target}"/>'/>
					</div>
					<div class="form-group">
						<label>운동 주의사항</label>
...
if(operation == "modify"){ 
frmModify.attr("action", "${contextPath}/workout/modify");
} else if(operation == "remove"){ //상품 정보 삭제 요청
frmModify.attr("action", "${contextPath}/workout/remove");
} else if(operation == "list"){ //상품 목록 화면 요청
frmModify.attr("action","${contextPath}/workout/list").attr("method","get");
frmModify.empty();
}
